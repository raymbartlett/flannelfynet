...
def spotify_results(token):
    ...
    user = Spotify(token)
    user.get_saved_albums()
    scored_albums, unscored_albums = get_user_scores(user.saved_albums)

    if len(user.saved_albums) < 1:
        return render_template('error.html', message="you have no saved albums")

    if len(scored_albums) < 1:
        return render_template('error.html', message="you have no scored albums")

    general = str(user.total_albums) + ' albums in total | ' +  str(len(scored_albums)) + ' albums with a score'

    ordered_scores = by_score(scored_albums)
    alphabetical_scores = by_artist(ordered_scores)
    unscored_albums = by_artist_unscored(unscored_albums)
